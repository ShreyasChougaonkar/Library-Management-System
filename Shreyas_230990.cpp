...
void User :: display_menu(){
    char c;
    cout<<"---------------------------------------------------------------------\n";
    cout<<"\n\nWelcome to the Software Interface of the IITK Library!\n\n";
...
void User :: login(){
    ...
    string id,password;
    cout<<"Enter your unique user id : ";
    cin>>id;
    cout<<"Enter password : ";
    cin>>password;
...
void User :: issue_book(string id,string bookname,string type_user){
    ...
    int fl=0;
    if(type_user=="1"){
        if(calc_fine(id, type_user)!=0){
            cout<<"You have overdue fines. First pay them then only you are eligible to borrow other books."<<endl;
        }
        else{
        int count=0;
        content.clear();
    ...
    if(type_user=="2"){
        ...
        for(int i=0;i<content.size();i++){
            if(content[i][0]==id){
                int curtime = time(0);
                int isstime = stoi(content[i][3]);
                if((curtime-isstime)/86400>60) fine+=0*((curtime-isstime)/86400 - 60);
            }
        }
        cout<<"Your fine is : "<<fine<<", faculties are not charged with any fine.""\n";
        content.clear();
    }
    return fine;
}
...
void Librarian :: display_lib_menu(string id){
    ...
    cout<<"Press 6 to reserve a book to read in library only, hence deleting the book from the data.\n";
